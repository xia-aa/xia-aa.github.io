...
description: >-
  Electric provides an Typescript client for streaming Shapes from Postgres into
  the web browser and other Javascript environments.
---

The TypeScript client is a higher-level client interface that wraps the [HTTP API](/docs/sync/api/http) to make it easy to sync [Shapes](/docs/sync/guides/shapes) in the web browser and other JavaScript environments.
...
**→ See the [authentication guide](/docs/sync/guides/auth) for a detailed explanation and complete implementation examples of the API proxy pattern.**
...
For authentication patterns including token refresh and authorization, see the [authentication guide](/docs/sync/guides/auth) which covers both proxy and gatekeeper authentication patterns in detail.
...
See the [Demos](/sync/demos/) and [integrations](/docs/sync/integrations/react) for more usage examples.
